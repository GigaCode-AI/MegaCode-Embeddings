"""
Train retriever without hard negatives
"""
import json
import os
import logging
...
# from torch.distributed.optim import ZeroRedundancyOptimizer  # import for 7b model
...
def infonce_loss(q, p, tau: float, compute_stats: bool = False, **kwargs):
    # scores
    qp = q @ p.T / tau
...
def clip_loss(q, p, tau: float, compute_stats: bool = False, **kwargs):
    """
...
def qwen3_emb_loss(q, p, m: float, tau: float, compute_stats: bool = False, **kwargs):
    """
    https://arxiv.org/pdf/2506.05176

    m - by how much negative score must exceed positive score to treat the negative as false
    tau - temperature
    """
    # scores
    qp = q @ p.T
    ...
    mask_qp = (qp > (qp.diag()[:, None] + m)).to(q)
    mask_qq = torch.eye(q.shape[0], device=q.device, dtype=q.dtype)
    scores = torch.cat([qp - mask_qp * 10000.0, qq - mask_qq * 10000.0, pp - mask_qq * 10000.0], dim=1) / tau
    labels = torch.arange(q.shape[0], device=q.device)
    ...
    if compute_stats:
        stats.update({f"loss_{k}": v for k, v in describe(loss).items()})
    stats["mask_qp_sum"] = mask_qp.sum().item()
    stats["mask_qq_sum"] = mask_qq.sum().item()
    return loss.mean(), stats


def gte_loss(q, p, tau: float, compute_stats: bool = False, **kwargs):
    """
    ...
    loss_p = loss_fn(torch.cat([qp.T, pp - mask_eye * 10000.0], dim=1) / tau, labels)
...
def main(args):
    ...
    # setup logger
    logger.info(f"nce batch: {gbs} queries, {gbs} candidates per query")
    ...
    def vectorize(input_ids, attention_mask):
        """Run forward, take eos vector, normalize"""
    ...
    def save(step: int):
        ...
        logger.info(f"[step {global_step}] saving checkpoint to {checkpoint_dir}")
    ...
    loss_fn = partial(loss_map[args.loss_fn], tau=args.tau, m=args.margin)
    ...
    for global_step in tqdm.trange(1, args.num_steps + 1, disable=local_rank != 0):
        ...
        for i in range(grad_acc_steps):
            # Sync gradients only on the last accumulation step; ~5% training speedup
            context = nullcontext if i == grad_acc_steps - 1 else model.no_sync
            with context():
                with torch.autocast("cuda", dtype=dtype):  # this context is only needed for the forward
                    qc = q.clone()
                    pc = p.clone()
                    b = micro_batches[i]
                    qc[i * mbs + o:(i + 1) * mbs + o] = vectorize(b["input_ids_q"], b["attention_mask_q"])
                    pc[i * mbs + o:(i + 1) * mbs + o] = vectorize(b["input_ids_p"], b["attention_mask_p"])

                    # z = qc @ pc.T
                    # # mask examples where negative score differs a lot from positive score
                    # thresholds = z[labels, labels] + args.margin
                    # mask_row = (z > thresholds[:, None]).to(z)
                    # per_example_loss_row = loss_fn((z - mask_row * 1e4) / args.tau, labels)
                    # mask_col = (z > thresholds[None]).to(z)
                    # per_example_loss_col = loss_fn((z - mask_col * 1e4).T / args.tau, labels)
                    # per_example_loss = per_example_loss_row + per_example_loss_col  # used to average
                    loss, stats = loss_fn(qc, pc, compute_stats=i == 0)
        ...
        if (global_step == 1) or (global_step % args.log_steps == 0):  # want first log after first step
            ...
            maybe_write_scalar("train/grad_norm", grad_norm.item(), global_step)
...
if __name__ == "__main__":
    ...
    g.add_argument("--save_only_model", type=str2bool, default=True)

    g = parser.add_argument_group("loss")
    g.add_argument("--loss_fn", type=str, choices=["infonce", "clip", "qwen3_emb", "gte"], default="clip")
    g.add_argument("--tau", type=float, default=0.01)
    # g.add_argument("--label_smoothing", type=float, default=0.0)
    g.add_argument("--margin", type=float, default=1.0,
                   help="By how much cos(q, neg) must exceed cos(q, pos) to treat neg as a false negative. "
                        "qwen3-emb uses 0.1: https://arxiv.org/pdf/2506.05176. "
                        "If >= 1, false negatives are not filtered."
                   )
