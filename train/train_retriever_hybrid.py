...
def ensure_eos(ids: List[int], eos: int):
    if ids[-1] != eos:
        return ids + [eos]
    return ids


def pad_sequences(sequences: List[List[int]], pad: int) -> Tuple[torch.Tensor, torch.Tensor]:
    input_ids = []
    attention_mask = []
...
def train_batches_gen(
    ...
    def yield_next() -> Tuple[List[int], List[int], List[int]]:
        ...
        return ensure_eos(q, eos), ensure_eos(p, eos), ensure_eos(n, eos)
    ...
    def sample():
        ...
        res = dict()
        res["input_ids_q"], res["attention_mask_q"] = pad_sequences(questions, pad)
        res["input_ids_p"], res["attention_mask_p"] = pad_sequences(pos, pad)
        res["input_ids_n"], res["attention_mask_n"] = pad_sequences(neg, pad)
...
def train_batches_gen_v2(
    ...
    def sample():
        ...
        for line_ in rng.sample(data, batch_size):
            ...
            p = rng.choice(x["positives"])["context"][:max_tokens_code - 1]
            n = rng.choice(x["negatives"])["context"][:max_tokens_code - 1]
            questions.append(ensure_eos(q, eos))
            pos.append(ensure_eos(p, eos))
            neg.append(ensure_eos(n, eos))
...
def train_batches_gen_kl(
    ...
    def sample():
        ...
        for line_ in rng.sample(data, batch_size):
            ...
            q = x["question"][:max_tokens_query - 1]
            questions.append(ensure_eos(q, eos))
            for d in rng.sample(x["candidates"], num_candidates):
                candidates.append(ensure_eos(d["context"][:max_tokens_code - 1], eos))
                teacher_scores.append(d["score"])

        res = dict()
        res["input_ids_q"], res["attention_mask_q"] = pad_sequences(questions, pad)
        res["input_ids_c"], res["attention_mask_c"] = pad_sequences(candidates, pad)
        res["teacher_scores"] = torch.tensor(teacher_scores)
        res = {k: v.to(device) for k, v in res.items()}
        return res
...
def clip_loss(q, p, n, tau: float, compute_stats: bool = False):
    ...
    # Here I sum instead of averaging, because the row loss dominates
    # over the column loss due to hard negatives
...
def gte_loss(q, p, n, tau: float, compute_stats: bool = False):
    ...
    # here we also sum instead of averaging, because loss_q strongly dominates over loss_p
    loss = loss_q + loss_p
...
def kl_loss(q, c, teacher_scores, tau_student, tau_teacher):
    q = q[:, None, :]  # [n, 1, d]
    c = c.reshape(q.shape[0], c.shape[0] // q.shape[0], -1).transpose(1, 2)  # [nk, d] -> [n, k, d] -> [n, d, k]
    student_scores = (q @ c).squeeze(1)  # [n, 1, d] x [n, d, k] = [n, 1, k] -> [n, k]
    teacher_scores = teacher_scores.to(q).reshape(q.shape[0], -1)  # [nk] -> [n, k]
    loss_fn = torch.nn.KLDivLoss(reduction="batchmean", log_target=True)
    log_probs_student = torch.log_softmax(student_scores / tau_student, dim=-1)
    log_probs_teacher = torch.log_softmax(teacher_scores / tau_teacher, dim=-1)
...
def main(args):
    ...
    def vectorize(input_ids, attention_mask):
        """Run forward and take the eos vector."""
    ...
    paths = [x for i, x in enumerate(get_paths(args.train_data_path_kl)) if i % world_size == local_rank]
    logger.info(f"num train paths kl: {len(paths)}")
    batches_gen_kl = train_batches_gen_kl(
        paths=paths,
        tokenizer=tokenizer,
        batch_size=args.per_device_train_batch_size_kl,
        max_tokens_query=args.max_tokens_query,
        max_tokens_code=args.max_tokens_code,
        num_candidates=args.num_candidates_kl,
        seed=args.seed,
        rank=local_rank,
        device=device
    )
    ...
    grad_acc_steps_kl = args.gradient_accumulation_steps_kl
    for global_step in tqdm.trange(1, args.num_steps + 1, disable=local_rank != 0):
        ...
        for i in range(grad_acc_steps):
            ...
            with context():
                ...
                loss = loss * float(world_size) * args.w_contrastive
                loss = scaler.scale(loss)
                loss.backward()
            # the same loss is computed at each step, so it only makes sense to compute statistics once
            if i == 0:
        ...
        model.train()
        loss_value_kl = 0.0
        for i in range(grad_acc_steps_kl):
            b = next(batches_gen_kl)
            context = nullcontext if i == grad_acc_steps_kl - 1 else model.no_sync
            with context():
                with torch.autocast("cuda", dtype=dtype):
                    q = vectorize(b["input_ids_q"], b["attention_mask_q"])
                    c = vectorize(b["input_ids_c"], b["attention_mask_c"])
                    loss = kl_loss(
                        q, c, b["teacher_scores"], tau_student=args.tau_student, tau_teacher=args.tau_teacher
                    )
                loss_value_kl += loss.item()
                loss = loss / grad_acc_steps_kl * args.w_kl
                loss = scaler.scale(loss)
                loss.backward()
        loss_value_kl /= grad_acc_steps_kl

        # update weights
        # 1. clip gradients:
        # 1.1. divide gradients by scale_factor
        ...
        # 3. if weights were updated, update the scheduler
        ...
        if (global_step == 1) or (global_step % args.log_steps == 0):  # we want the first log after the first step
            for k, v in stats_loss.items():
                maybe_write_scalar(f"train/{k}", v, global_step)
            maybe_write_scalar(f"train/loss_kl", loss_value_kl, global_step)
...
if __name__ == "__main__":
    ...
    g.add_argument("--train_data_path_kl")
    ...
    g.add_argument("--per_device_train_batch_size_kl", type=int, default=1)
    ...
    g.add_argument("--gradient_accumulation_steps_kl", type=int, default=1)
    ...
    g = parser.add_argument_group("contrastive_loss")
    g.add_argument("--loss_fn", type=str, choices=["clip", "gte"], default="clip")
    g.add_argument("--tau", type=float, default=0.01)
    g.add_argument("--w_contrastive", type=float, default=1.0)  # this loss is much lower in magnitude than kl

    g = parser.add_argument_group("kl_loss")
    g.add_argument("--num_candidates_kl", type=int, default=16)
    g.add_argument("--tau_student", type=float, default=0.05)
    g.add_argument("--tau_teacher", type=float, default=1.0)
    g.add_argument("--w_kl", type=float, default=1.0)
